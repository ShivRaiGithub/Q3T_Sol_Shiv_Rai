...
pub struct ResolveBet<'info>{
  ...
  impl<'info>ResolveBet<'info>{
        ...
        pub fn verify_ed25519_signatuer(&mut self,sig:&[u8])->Result<()>{
            ...
            require!(&signature.signature.ok_or(DiceError::Ed25519Message)?.eq(sig), DiceError::Ed25519Signature);
        ...
        pub fn resolve_bet(&mut self, bumps: &ResolveBetBumps, sig: &[u8])->Result<()>{
            ...
            if self.bet.roll>roll{
                let payemnt = (self.bet.amount as u128)
